<!-- Top  -->
<div class="clTab" id="itemList">

	<div id="grid"></div>

	<div id="inputWindow">
		<form method='POST'
			action="${g.createLink(controller:'item',action:'saveImage')}"
	...
	<script>
			...
			$(document).ready(function () {
                    ...
                    var dataSource = new kendo.data.DataSource({
                            transport: {
                                read:  {
                                    url: "${g.createLink(controller:'item',action:'search')}",
                                    type: "POST",
                                    dataType: "jsonp",
                                    contentType: 'application/json'
                                },
                                update: {
                                    url: "${g.createLink(controller:'item',action:'upsert')}",
                                    type: "POST",
                                    dataType: "jsonp",
                                    contentType: 'application/json'
                                },
                                destroy: {
                                    url: "${g.createLink(controller:'item',action:'delete')}",
                                    type: "POST",
                                    dataType: "jsonp",
                                    contentType: 'application/json'
                                },
                    ...
                    $("#grid").kendoGrid({
                        ...
                        toolbar: ["create"],
                        columns: [
							{ title: "Item", width: 25, template: "#= imageFunc(data) #" },      
                ...
                $("#submitIFrame").load(function() {
					  ...
					  var dataSource = $("#grid").data("kendoGrid").dataSource;
    			  	  ...
    			  	  dataSource._data[selectedItemId]['imageUUID']=parsedResponse.UUID;
    			  	  dataSource._data[selectedItemId]['dirty']=true;
    			  	  dataSource.data(dataSource._data);

					  //Put the row in edit mode
    			  	  var uid = dataSource._data[selectedItemId]['uid'];
               ...
               function openInputWindow(clickedImage){
                    ...
                    var selectedItemId = row.index();
                    log.info ("The select data grid row index is " + selectedItemId);
              ...
              //with the image
                function imageFunc(model) {
                    ...
                    var html = "<div class='clImage' id='itemImage'><a href='#'><img class='clImage' id='itemImage' onclick='openInputWindow($(this));return false;' src='" + file + "' /></a></div>";
